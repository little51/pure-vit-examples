...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        b = x.shape[0]
        # 卷积投影: [B, C, H, W] -> [B, emb_size, H/patch_size, W/patch_size]
        x = self.projection(x)
        # 展平空间维度: [B, emb_size, grid_h, grid_w] -> [B, emb_size, num_patches]
        x = x.flatten(2)
        # 转置: [B, emb_size, num_patches] -> [B, num_patches, emb_size]
        x = x.transpose(1, 2)
