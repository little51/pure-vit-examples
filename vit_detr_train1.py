...
from vit_detr_model1 import DETR
...
def main():
    ...
    # 模型
    model = DETR(num_classes=4, num_queries=100,num_heads=6, emb_size=384).to(device)
    print(f'参数量: {sum(p.numel() for p in model.parameters()):,}')
    # 优化器
    optimizer = AdamW(model.parameters(), lr=1e-5, weight_decay=1e-5)
    scheduler = StepLR(optimizer, step_size=30, gamma=0.1)
    # 损失函数
    matcher = SimplifiedHungarianMatcher(cost_class=1, cost_bbox=2)
    weight_dict = {'loss_ce': 1, 'loss_bbox': 2}
    criterion = SimplifiedSetCriterion(4, matcher, weight_dict, eos_coef=0.1).to(device)
    # 训练循环
    best_loss = float('inf')
